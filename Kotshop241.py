...
# ─── Каталог товаров ───
# Все товары теперь разрешены к покупке
ALLOWED_PRODUCTS = {
    "60uc", "120uc", "180uc", "240uc",
    "325uc", "385uc", "445uc",
    "660uc", "720uc", "985uc",
    "1320uc", "1800uc", "1920uc",
    "2125uc", "2460uc", "3850uc",
    "4510uc", "5650uc",
}
...
PRODUCTS = {
    ...
    "325uc":  {"name": "325 UC",  "price": 422,  "amount_kopecks": 422  * 100, "deliveries": ["325_uc"]},
    "385uc":  {"name": "385 UC",  "price": 508,  "amount_kopecks": 508  * 100, "deliveries": ["325_uc", "60_uc"]},
    "445uc":  {"name": "445 UC",  "price": 595,  "amount_kopecks": 595  * 100, "deliveries": ["60_uc", "60_uc", "325_uc"]},
    "660uc":  {"name": "660 UC",  "price": 845,  "amount_kopecks": 845  * 100, "deliveries": ["660_uc"]},
    "720uc":  {"name": "720 UC",  "price": 931,  "amount_kopecks": 931  * 100, "deliveries": ["660_uc", "60_uc"]},
    "985uc":  {"name": "985 UC",  "price": 1267, "amount_kopecks": 1267 * 100, "deliveries": ["660_uc", "325_uc"]},
    "1320uc": {"name": "1320 UC", "price": 1690, "amount_kopecks": 1690 * 100, "deliveries": ["660_uc", "660_uc"]},
    "1800uc": {"name": "1800 UC", "price": 2122, "amount_kopecks": 2122 * 100, "deliveries": ["1800_uc"]},
    "1920uc": {"name": "1920 UC", "price": 2301, "amount_kopecks": 2301 * 100, "deliveries": ["1800_uc", "60_uc", "60_uc"]},
    "2125uc": {"name": "2125 UC", "price": 2545, "amount_kopecks": 2545 * 100, "deliveries": ["1800_uc", "325_uc"]},
    "2460uc": {"name": "2460 UC", "price": 2965, "amount_kopecks": 2965 * 100, "deliveries": ["1800_uc", "660_uc"]},
    "3850uc": {"name": "3850 UC", "price": 4202, "amount_kopecks": 4202 * 100, "deliveries": ["3850_uc"]},
    "4510uc": {"name": "4510 UC", "price": 5044, "amount_kopecks": 5044 * 100, "deliveries": ["3850_uc", "660_uc"]},
    "5650uc": {"name": "5650 UC", "price": 6314, "amount_kopecks": 6314 * 100, "deliveries": ["1800_uc", "3850_uc"]},
}
...
# ─── Файл для защиты от повторной отправки товара ───
DELIVERED_FILE = os.getenv("DELIVERED_FILE", "delivered_orders.json")
DELIVERED_CLEANUP_DAYS = 7
...
def load_delivered_orders():
    ...
    try:
        with open(DELIVERED_FILE, "r", encoding="utf-8") as f:
            data = json.load(f)
        now = time.time()
        cutoff = now - DELIVERED_CLEANUP_DAYS * 86400
        delivered_orders = {oid for oid, ts in data.items() if ts > cutoff}
        logger.info(f"Загружено {len(delivered_orders)} доставленных заказов из файла {DELIVERED_FILE}")
    except FileNotFoundError:
        delivered_orders = set()
        logger.info(f"Файл {DELIVERED_FILE} не найден — стартуем с пустым множеством")
    except Exception as e:
        delivered_orders = set()
        logger.error(f"Не удалось загрузить delivered_orders: {e}")
...
def mark_delivered(order_id: str):
    ...
    try:
        ...
        try:
            ...
            pass
        data[order_id] = time.time()
        with open(DELIVERED_FILE, "w", encoding="utf-8") as f:
            json.dump(data, f, ensure_ascii=False, indent=2)
    except Exception as e:
        logger.error(f"Не удалось записать delivered_orders: {e}")
...
# ─── Отправка отзыва в группу через send_message ───
async def send_review_to_group(rating: int, review_text: str = "", user_name: str = "") -> bool:
    ...
    header = "📝 Новый отзыв\n\n"
    rating_line = f"Оценка: {rating}/10 {stars}\n"
    ...
    if user_name:
        author = f"\n— @{user_name}" if not user_name.startswith("@") else f"\n— {user_name}"
    else:
        author = ""

    full_text = header + rating_line + body + author
    ...
    try:
        ...
        logger.info(f"Отзыв отправлен в группу {REVIEW_CHAT_ID}: rating={rating}, has_text={bool(review_text)}")
        return True
    except Exception as e:
        logger.error(
            f"Не удалось отправить отзыв в группу {REVIEW_CHAT_ID}: "
            f"{type(e).__name__}: {e}"
        )
        return False
...
async def process_paid_order(order_id: str, info: dict):
    ...
    # ── Проверка от повторной отправки товара ──
    if order_id in delivered_orders:
        logger.warning(f"Заказ {order_id} уже был обработан — повторная отправка отменена")
        return
    ...
    logger.info(f"Отправка товара для {order_id} завершена")
...
# ─── Тестовая обработка «оплаченного» заказа (без доставки) ───
...
async def cb_review_start(callback, state: FSMContext):
    """Начало процесса отзыва — запрос оценки."""
    await state.clear()
...
async def process_rating(message, state: FSMContext):
    ...
    existing_review_text = existing_data.get("review_text", "")

    await state.update_data({
        "rating": rating,
    })

    if existing_review_text:
        await message.answer(
...
async def cb_review_write(callback, state: FSMContext):
    """Переход к написанию текста отзыва."""
...
async def process_review_text(message, state: FSMContext):
    ...
    if not message.text:
        await message.answer("❌ Пожалуйста, отправьте текст отзыва текстовым сообщением.")
        return
    ...
    logger.info(f"Отзыв: rating={rating}, text='{review_text[:50]}...'")
...
async def cb_review_send(callback, state: FSMContext):
    """Отправка отзыва (оценка + текст) в группу."""
    data = await state.get_data()

    logger.info(f"review_send: данные из state = {data}")
    ...
    if callback.from_user.username:
        ...
        user_name = f"id:{callback.from_user.id}"

    ok = await send_review_to_group(rating, review_text, user_name)

    if not ok:
        logger.error("review_send: отправка отзыва в группу неудачна")
        await callback.message.answer(
...
async def cb_review_edit_text(callback, state: FSMContext):
    """Изменение текста отзыва — оценка сохраняется."""
    data = await state.get_data()
    rating = data.get("rating", 5)
    await state.set_state(OrderFlow.waiting_for_review_text)
...
async def cb_review_change_rating(callback, state: FSMContext):
    """Изменение оценки — текст отзыва сохраняется."""
    data = await state.get_data()
    review_text = data.get("review_text", "")
    await state.set_state(OrderFlow.waiting_for_rating)
...
async def cb_review_send_stars_only(callback, state: FSMContext):
    """Отправка только оценки (без текста отзыва) в группу."""
    data = await state.get_data()

    logger.info(f"review_send_stars_only: данные из state = {data}")
    ...
    ok = await send_review_to_group(rating, "", user_name)

    if not ok:
        logger.error("review_send_stars_only: отправка оценки в группу неудачна")
        await callback.message.answer(
            "❌ Не удалось отправить отзыв. Попробуйте позже или напишите в поддержку."
        )
    else:
